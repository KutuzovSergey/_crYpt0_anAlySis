...
import {
  GetAllListType,
  ResultAllListType,
} from "../type/typeAP/typesGetCoins";
import {
  DataObjType,
  DataProcessingType,
} from "../type/typeUtils/typesFileCheck";

export async function getAllList(): Promise<GetAllListType> {
  const api_key: string =
    "?api_key=92c340e1dee1b05551b8fe09fb59f2bc6ba4715e3ec434f226370c7654de8b38";
  const url: string = "https://min-api.cryptocompare.com/data/blockchain/list";
  const get: string = "GET";

  const result: ResultAllListType = await RequestServer.getData(
    get,
    url,
    api_key
  );
...
export async function getListOnPage(
  coins: string[]
): Promise<DataProcessingType | void> {
  if (typeof coins === "undefined" || !Array.isArray(coins) || !coins.length) {
    coins = [
      "RMESH",
      "ADA",
      "SAL",
  ...
  const result: DataObjType = await RequestServer.getData(
    get,
    url,
    parameters,
    api_key
